...
<div class="content-wrapper" style="zoom:90%;">
    ...
    <div class="content-header">
        <div class="container-fluid">
            <div class="row mb-2">
                <div class="col-sm-6">
                    <h1 class="m-0 text-dark">Target Setting</h1>
                </div><!-- /.col -->
                <div class="col-sm-6">
    ...
    <div class="container">
        <div class="card">
            <div class="card-body table-responsive">
                <table id="agent_target_setting_table" class="table table-striped table-bordered  bg-light">
                    <thead>
                    <tr>
            ...
            <div class="modal-dialog modal-lg" role="document">
                <div class="modal-content">
                    <div class="modal-header">
                        <h5 class="modal-title" id="myModalLabel">Agent Target Setting</h5>
                        <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                            <span aria-hidden="true">&times;</span></button>
                    </div>
                    <div class="modal-body">
                        <button type="button" class="btn btn-primary btn-sm" style="float: right">Excel Upload</button>
                        <button type="button" class="btn btn-secondary btn-sm" id ="target_set_modal_button" style="float: right; margin-right: 5px;">Set Agent Target</button>

                        <div class="card-body table-responsive">
                            <table  id="agent_sub_target_table" class="table table-border" style="width:100%">
                                <thead>
                                <tr>
                                    <th>ID</th>
                                    <th>Date</th>
                    ...
                    <div class="modal-body">
                        ...
                        <form name="set_target_setting_form" id="set_target_setting_form" method="post" action="#" >
                            <div class="form-group">
                                ...
                                <input type="hidden" id="agent_target_id" >
                            </div>
                            <div class="form-group">
                ...
                <div class="modal-content">
                    ...
                    <div class="modal-body">
                        ...
                        <form name="cat_add_form" id="cat_add_form" method="post" action="#" >
                            <div class="form-group">
                            <input type="hidden" id="cat_agent_id">
...
<script>
    ...
    $('#agent_sub_target_table').on('click', 'a.view_categaory', function(e) {
        e.preventDefault();
        const tableStore = $('#agent_sub_target_table').DataTable();
        const tr=  $(this).parents('tr');
        const row_id = tableStore.row( tr ).data().id;
        const agent_id = tableStore.row( tr ).data().agent_id;
        ...
        var cat_table = $("#catTable").DataTable({
            ...
            ajax:{
                url:  "<?php echo base_url('index.php/sm/category/get-target-setting-category'); ?>",
                type: "POST",
                data:{
                    target_id:row_id,
                    agent_id:agent_id
    ...
    $('#catTable').on('click', 'a.cat_delete', function (e) {
        ...
        $('#deleteButton').on('click', function(e) {
            ...
            $.ajax({
            url:  "<?php echo base_url('index.php/sm/category/target-setting-category-delete'); ?>",
            data: "id="+row_id,
            type: "POST",
            dataType: "JSON",
            cache: false,
            ...
            {
                alert("Agent category target deleted successfully.");
                $('#catTable').DataTable().draw();
            }
            });
        });
    });
    $('#agent_sub_target_table').on('click', 'a.set_target_category', function (e) {
        e.preventDefault();
        const tableStore = $('#agent_sub_target_table').DataTable();
        const tr =  $(this).parents('tr');
        const row_id = tableStore.row( tr ).data().id;
    ...
    $("#cat_add_form").validate({
        ...
        submitHandler: function (form){
            ...
            $.ajax({
                url:  "<?php echo base_url('index.php/sm/category/category-mappping-add'); ?>",
                data:  
                {
                    agent_id:agent_id,
                    store_id:store_id,
                ...
                {
                    if(response.status){
                        $('.cat_error_div').html("<div class='alert alert-success'>Agent category target created successfully.</div>");
    ...
    var targetTable = $("#agent_target_setting_table").DataTable({
        "bLengthChange": true,
        "bPaginate": true,
        "bInfo": true,
        "autoWidth": false,
        ...
        aoColumns: [
            ...
            }, { mData: 'updated_at',width:"20%" },
            {
                ...
                data: null,
                className: "center",
                defaultContent: '<center><a href="" class="fas fa-eye  view_agent_target_setting" ></a></center>'
            }
        ],
    });
    $('#agent_target_setting_table').on('click', 'a.view_agent_target_setting', function(e) {
        e.preventDefault();
        const tableStore = $('#agent_target_setting_table').DataTable();
        const tr=  $(this).parents('tr');
        const row_store_id = tableStore.row( tr ).data().store_id;
        const row_id = tableStore.row( tr ).data().id;
        const user_id = tableStore.row( tr ).data().user_id;
        $('#store_id').val(row_store_id);
        $('#agent_target_id').val(row_id);
        $('#agent_sub_target_table').DataTable({
            "bLengthChange": true,
            "destroy": true,
            "bPaginate": true,
            "bInfo": true,
            ...
            ajax:{
                url:  "<?php echo base_url('index.php/sm/agent/get-set-target-griddata'); ?>",
                type: "POST",
                data:{
                    id:row_id,
    ...
    $("#set_target_setting_form").validate({
        ...
        submitHandler: function (form){
            ...
            var agent_id = $('#agent_target_id').val();
            var target_date =$('#Target_Date').val();
            var Total_Sales_Value =$('#Total_Sales_Value').val();
            var Total_Units_Sold =$('#Total_Units_Sold').val();
            $.ajax({
                type: "POST",
                url: "<?php echo base_url('index.php/sm/agent/agent-set-target-setting'); ?>",
                data:  
                {
                    agent_id:agent_id,
                    store_id:store_id,
                ...
                success: function (response) {
                    if(response.status){
                        $('.target_error_div').html("<div class='alert alert-success'>Agent target created successfully.</div>");
                        $("#set_target_setting_form")[0].reset();
                        setTimeout(function (){
                            $(".target_error_div").html("");
                            $("#target_set_modal").modal("toggle");
                            $('#agent_sub_target_table').DataTable().draw();
                        },2500)
